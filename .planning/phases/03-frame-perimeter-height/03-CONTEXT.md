...
- **D-03 [informational, deferred]:** **Tessellated-vertex measurement is the documented upgrade path**, not v1 scope — only pursue it if bounding-box overestimation proves inadequate (verify magnitude against real CAD, see discretion note). Deliberate non-goal for Phase 3 (no plan implements it); tracked here as the future upgrade path.
